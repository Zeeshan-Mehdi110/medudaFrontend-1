      ...
      <div>
        <div className={ isOpen ? "block" : "hidden"}>
          { !paidByGiftcard && cart?.payment_sessions?.length ? (
            <>
              ...
              {isStripe && stripeReady && (
               ...
               {cart.payment_session?.provider_id === "manual" &&(
                <div className="mt-5 transition-all duration-150 ease-in-out">
                <Form cart={cart} onTransactionComplete={handleTransactionComplete} items={cart.items} shippingTotal={(((cart.shipping_total ?? 0) + (cart.shipping_tax_total ?? 0)) / 100) ?? 0} />
                </div>
              )}
                  {cart.payment_session?.provider_id === "paypal" &&(
                <div className="mt-5 transition-all duration-150 ease-in-out">
          ...
          <Button
            ...
            disabled={
              (isStripe && !cardComplete) ||
              (!cart?.payment_session && !paidByGiftcard) || !isPaidSuccessfully
            }
            data-testid="submit-payment-button"
          >
            {t("continue-to-review")}
