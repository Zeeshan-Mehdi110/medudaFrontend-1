...
//         setTimeout(() => {
//           toast.error("No session found, please login again, redirected to login...");
//           signOut()
//         }, 3000)
//       } else {
//         console.error(error)
...
const MyImagesComponent: React.FC<MyImagesComponentProps> = ({
  ...
  const [zoom, setZoom] = useState(1) // Start zoom at 0.5 to allow zooming out
  ...
  function getCookieValue(name: any) {
    ...
    if (parts.length === 2) {
      ...
      if (cookieValue) {
        console.log(cookieValue.split(";").shift())
  ...
  const handleCropImage = async () => {
    ...
    try {
      const croppedBase64 = await getCroppedImg(URL.createObjectURL(file), croppedArea, 48 / 67.5);
      const base64data = croppedBase64.replace(/^data:image\/jpeg;base64,/, ''); // Remove the prefix
    ...
    <div>
              ...
              <div key={index} className="relative group">
                ...
                {loadedImages[index as keyof typeof loadedImages] && (
                  <div className="flex gap-2 absolute top-1 right-1 z-20">
                    ...
                    >
                      {current !== "delete-image-button" ? (
                        <Trash className="opacity-100 pointer-events-none" />
                      ) : (
                        <CustomSpinner />
                      )}
                    </Button>
                    ...
                    >
                      {current !== "add-to-cart" ? (
                        <ShoppingBag className="opacity-100 pointer-events-none" />
                      ) : (
                        <CustomSpinner />
                      )}
                    </Button>
      ...
      >
        ...
        <label
          htmlFor="file-upload"
          style={{ fontSize: "1rem" }}
          className="flex-col cursor-pointer w-full text-blue-700 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-100 file:text-blue-800 hover:file:bg-blue-200 flex items-center justify-center gap-2"
        >
          <div className="flex items-center justify-center w-10 h-10 bg-blue-100 rounded-full">
            <ArrowUpMini className="animate-flicker w-5 h-5 text-blue-800" />
          </div>
        ...
        >
          <div className="flex items-center justify-center min-h-screen px-4">
            ...
            <div className="relative bg-white p-6 rounded-lg shadow-lg max-w-md w-full mx-auto z-50">
              ...
              <div className="mt-4 relative w-full h-64 bg-gray-100">
                <Cropper
                  ...
                  minZoom={0.3} // Set a lower limit for zoom, e.g., 0.3 for 30% scale
                  maxZoom={3} // You can adjust the max zoom if needed
                />
              </div>
              <div className="mt-4 flex flex-col items-center gap-2">
                ...
                />
                <div className="flex justify-center gap-3">
                  <Button variant={"danger"} onClick={() => setIsCropping(false)}>{t("cancel")}</Button>
                  <Button onClick={handleCropImage}>{t("crop")}</Button>
                </div>
              </div>
            </div>
          </div>
